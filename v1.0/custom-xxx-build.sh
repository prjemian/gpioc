...
cd "${SYNAPPS}/iocs/xxx_custom/iocBoot/iocxxx/"
pwd
ls -lAFgh
...
export dbFile=$(readlink -f ${SUPPORT}/iocStats*/db/iocAdminSoft.db)
...
export MOTOR=${MOTOR:-$(readlink -f ${SUPPORT}/motor*)}
patch "${MOTOR}/db/asyn_motor.db" "${USER_DIR}/db/asyn_motor.db.diffs"
...
# general purpose PVs
cp ${USER_DIR}/db/general_purpose.db ./substitutions/
...
echo   >> "${SUBFILE}"
echo file \"\${TOP}/iocBoot/iocxxx/substitutions/general_purpose.db\"  >> "${SUBFILE}"
echo {  >> "${SUBFILE}"
echo pattern  >> "${SUBFILE}"
echo {N}  >> "${SUBFILE}"
...
EOF
patch "${XXX}/xxxApp/op/ui/xxx.ui" "${diff_file}"
export diff_file=
...
# - - - - - - - - - - - - - - - - - - - - - - - -
# - - - - - - - - - - - - - - - - - - - - - - - - starter for custom XXX IOC
# - - - - - - - - - - - - - - - - - - - - - - - -
ln -s $(readlink -f "${SYNAPPS}/iocs/xxx_custom/iocBoot/iocxxx") ${USER_DIR}/xxx_custom_ioc

export starter=${USER_DIR}/start_custom_xxx_ioc.sh
cat > "${starter}" << EOF
#!/bin/bash
if [ "\$#" == "1" ]; then
    export PREFIX=\${1}
else
    export PREFIX=\${PREFIX:-xxx:}
fi
echo "Starting custom XXX IOC with PREFIX='\${PREFIX}'"
bash ./xxx_custom_ioc/softioc/xxx.sh start
EOF
chmod +x "${starter}"
